...
const validateCoordinate = (value: number) => {
  if (value === undefined || value === null) return undefined; // Убираем обязательность
...
const transform = (data: any) => {
  ...
  if (data.location?.coordinates) {
    transformed.longitude = data.location.coordinates[0] ?? "";
    transformed.latitude = data.location.coordinates[1] ?? "";
  } else {
...
// Функция для трансформации данных перед отправкой на сервер
const transformForServer = (data: any) => {
  const transformed = { ...data };

  // Преобразуем latitude/longitude обратно в location.coordinates только если оба значения заданы и не пустые
  if (
    transformed.latitude !== undefined &&
    transformed.longitude !== undefined &&
  ...
  // Удаляем отдельные поля lat/lng из объекта, если они есть
...
export const PlacesEdit = () => {
  ...
  const handleSave = (values: any) => {
    ...
    update(
      "places",
      { id: values.id, data: cleaned },
      {
        onSuccess: () => {
          notify("Place updated", { type: "success" });
